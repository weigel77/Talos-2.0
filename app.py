...
APOLLO_PRECHECK_CACHE_SECONDS = 20
...
def create_app(test_config: Optional[Dict[str, Any]] = None) -> Flask:
    ...
    def open_trade_management_page() -> str:
        ...
        management_payload = open_trade_manager.evaluate_open_trades(
            send_alerts=False,
            caller_source="route:/management/open-trades",
        )
...
def get_status_snapshot(market_data_service: MarketDataService, ticker: str, *, query_type: str) -> Dict[str, Any]:
    ...
    try:
        cache_key = _build_hosted_cache_key("status-snapshot", ticker=ticker, query_type=query_type)
        return _get_cached_hosted_payload(
            cache_key,
            ttl_seconds=HOSTED_HEADER_SNAPSHOT_CACHE_SECONDS,
...
def parse_performance_request_filters(source: Any) -> Dict[str, list[str]]:
    ...
    legacy_group_aliases = {
        ...
        "profile": "profiles",
        "result": "results",
        "trade_mode": "trade_modes",
        "macro_grade": "macro_grades",
        "structure_grade": "structure_grades",
    ...
    for key in PERFORMANCE_FILTER_GROUPS:
        ...
        if not values:
            alias_key = legacy_group_aliases.get(key)
            if alias_key:
                values = source.getlist(alias_key) if hasattr(source, "getlist") else source.get(alias_key, [])
...
def load_retained_trade_rows(store: TradeRepository, trade_mode: str) -> list[Dict[str, Any]]:
    requested_mode = str(trade_mode or "").strip().lower()
    normalized_mode = "talos" if requested_mode == "talos" else resolve_trade_mode(trade_mode)
    if normalized_mode == "talos":
        loaded_trade_modes = ("real", "simulated", "talos")
    elif normalized_mode == "real":
        loaded_trade_modes = ("real", "talos")
    elif normalized_mode == "simulated":
        loaded_trade_modes = ("simulated",)
    else:
        loaded_trade_modes = ("real", "simulated", "talos")
    loaded_trades = [trade for trade_mode_key in loaded_trade_modes for trade in store.list_trades(trade_mode_key)]
    retained_trades = [trade for trade in loaded_trades if is_retained_trade_system(trade)]
    if normalized_mode == "talos":
        return [trade for trade in retained_trades if resolve_trade_system_name(trade) == "Talos"]
    return retained_trades
...
def build_hosted_open_trade_management_payload(*, app: Optional[Flask] = None) -> Dict[str, Any]:
    return _get_cached_hosted_payload(
        ...
        builder=lambda: get_open_trade_manager(app).evaluate_open_trades(
            send_alerts=False,
            caller_source="route:/hosted/open-trades",
        ),
        app=app,
    )
...
def execute_apollo_precheck(apollo_service: ApolloService, trigger_source: str, *, force_refresh: bool = False) -> Dict[str, Any]:
    ...
    cache_key = "apollo:precheck"
    if force_refresh:
        _clear_hosted_payload_cache(cache_key)
        precheck_result = apollo_service.run_precheck(force_refresh=True, caller_source=trigger_source)
    else:
        precheck_result = _get_cached_hosted_payload(
            cache_key,
            ttl_seconds=APOLLO_PRECHECK_CACHE_SECONDS,
            builder=lambda: apollo_service.run_precheck(force_refresh=False, caller_source=trigger_source),
        )
